...
public class GenericHibernateDAO<T, ID extends Serializable> implements
	...
	public List<T> findAllLimit(Integer count, Integer posStart) {
		...
		Query q = session.createQuery(
				...
				.setMaxResults(count).setFirstResult(posStart);
	...
	public List<T> findByMask(String mask, Class<?> classe, String campo) {
		Query q = (Query) getSession().createQuery(
				" from " + classe.getSimpleName() + " where lower(" + campo
						+ ") like lower(?)");
		q.setString(0, "" + mask + "%");
		q.setMaxResults(10);
		return q.list();
	...
	public List<T> findByMask2(String mask, Class<?> classe, String campo) {
		Query q = (Query) getSession().createCriteria(classe.getClass())
				.add(
						Restrictions.ilike(campo, "%" + mask + "%",
								MatchMode.ANYWHERE));
		q.setString(0, "" + mask + "%");
	...
	public static List<Criterion> buildCriterionForRelationships( Object entity, String... excludeProperty )
	{
		...
		final ArrayList<String> arrayUtil = new ArrayList<String>();
	...
	@Override
	public List<T> getRegByExampleLimit(T example, Integer posStart,
			Integer maximo) {
		Criteria criteria = getSession().createCriteria(getPersistentClass());
